...
class EveryDays: NSObject, DictModelProtocol {
    ...
    class func loadEventsData(_ completion: (_ data: EveryDays?, _ error: NSError?)->()) {
        let path = Bundle.main.path(forResource: "events", ofType: nil)
        ...
        willSet {
            if let tmpDate = newValue {
                if tmpDate.length == 10 {
                    if let tmpM = Int(tmpDate.substring(with: NSRange(location: 5, length: 2))) {
                        switch tmpM {
                        case 1:
                            self.month = "Jan."
                        case 2:
                    ...
                    self.day = tmpDate.substring(with: NSRange(location: 8, length: 2))
                } else {
                    self.date = newValue
                    return
                }
...
class ThemeModels: NSObject, DictModelProtocol {
    ...
    class func loadThemesData(_ completion: (_ data: ThemeModels?, _ error: NSError?)->()) {
